...
`).join("");t.innerHTML=r,new u(".gallery a",{captions:!0,captionsData:"alt",captionDelay:250}).refresh()},l=document.querySelector(".form"),n=document.querySelector(".input-seach"),d=document.querySelector(".gallery"),c=document.querySelector(".js-loader");l.addEventListener("submit",async o=>{o.preventDefault();const t=n.value.trim();if(d.innerHTML="",t===""){f.error({position:"topLeft",message:"Sorry, the field must not contain only spaces. Please, try again!"}),l.reset();return}c.classList.remove("is-hidden");try{const r=await y(t);n.value="",c.classList.add("is-hidden"),r.hits.length!==0&&h(r.hits,d)}catch{c.classList.add("is-hidden"),n.value="",console.error("Error fetching images:",error)}});
